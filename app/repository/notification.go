...
func CreateNotification(data models.VANotification) (models.VANotification, error) {
	err := config.DB.Create(&data).Error

	return data, err
}

func GetNotificationByID(id uuid.UUID, preloadFields []string) (response models.VANotification, err error) {
...
func GetNotificationByIDs(ids []uuid.UUID, preloadFields []string) (responses []models.VANotification, err error) {
	db := utils.BuildPreload(config.DB, preloadFields)
...
func GetNotifications(param dto.FindParameter, preloadFields []string) (responses []models.VANotification, total int64, totalFiltered int64, err error) {
...
func UpdateNotification(data models.VANotification) (models.VANotification, error) {
	err := config.DB.Save(&data).Error

	return data, err
}

func DeleteNotification(data models.VANotification) error {
