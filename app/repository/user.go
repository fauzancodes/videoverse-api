...
func CreateUser(data models.VAUser) (models.VAUser, error) {
	err := config.DB.Create(&data).Error

	return data, err
}

func GetUserByID(id uuid.UUID, preloadFields []string) (response models.VAUser, err error) {
...
func GetUsers(param dto.FindParameter, preloadFields []string) (responses []models.VAUser, total int64, totalFiltered int64, err error) {
...
func UpdateUser(data models.VAUser) (models.VAUser, error) {
	err := config.DB.Save(&data).Error

	return data, err
}

func DeleteUser(data models.VAUser) error {
